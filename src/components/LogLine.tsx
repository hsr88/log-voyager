...
interface LogLineProps {
    ...
    onSelect: (lineNum: number, content: string, isSelected: boolean) => void;
    id: string;
...
const LogLine = ({ 
    ...
    if (level === 'error') { 
        lineStyle = 'border-l-2 border-red-500 bg-red-500/5 hover:bg-red-500/10'; 
        numStyle = 'text-red-500 font-bold cursor-pointer'; 
    } else if (level === 'warn') { 
        lineStyle = 'border-l-2 border-orange-400 bg-orange-400/5 hover:bg-orange-400/10'; 
        numStyle = 'text-orange-400 cursor-pointer'; 
    }
    
    if (isBookmarked) { 
        lineStyle += ' bg-[#ff00ff]/10'; 
        numStyle = 'text-[#ff00ff] font-bold cursor-pointer'; 
    }
    
    if (isSelected) {
        lineStyle += ' bg-[#00f3ff]/20';
    }
    ...
    const handleClick = () => {
        if (isSelecting) {
            onSelect(number, content, !isSelected);
        } else {
            onToggleBookmark(number, content);
        }
    };
    ...
    return (
        <div 
            id={id} 
            className={`flex ${fontSizeClass} font-jetbrains leading-relaxed py-0.5 ${lineStyle} transition-colors group`}
        >
            <div 
                className={`w-12 shrink-0 text-right pr-3 select-none flex justify-end gap-1 ${numStyle}`} 
                onClick={handleClick}
            >
                {isSelecting ? (
                    isSelected ? <Check size={10} className="mt-0.5 text-[#00f3ff]" /> : <div className="w-3 h-3 rounded border border-slate-600 mt-0.5" />
                ) : (
                    isBookmarked && <Bookmark size={10} className="mt-0.5 fill-current" />
                )}
                {number}
            </div>
            <div className={`flex-1 min-w-0 ${wrapClass}`}>
                ...
                {jsonMatch && (
                    <div className="inline-block ml-2">
                        <button 
                            ...
                            className="text-[9px] uppercase font-bold text-[#00f3ff] hover:text-white border border-[#00f3ff]/30 px-1.5 rounded hover:bg-[#00f3ff]/20 transition-all"
                        >
                            {isExpanded ? 'RAW' : 'JSON'}
                        </button>
                    </div>
                ...
                {isExpanded && (
                    <pre className="mt-2 p-3 bg-black/80 rounded border border-white/10 text-[#00f3ff] overflow-x-auto shadow-2xl whitespace-pre">
